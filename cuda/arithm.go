...
//
// For further details, please see:
// https://docs.opencv.org/4.x/de/d09/group__cudaarithm__core.html#gaac939dc3b178ee92fb6e7078f342622c
func Merge(mv []GpuMat, dst *GpuMat) {
	cMatArray := make([]C.GpuMat, len(mv))
	for i, r := range mv {
		cMatArray[i] = r.p
	}
	cMats := C.struct_GpuMats{
...
// MergeWithStream creates one multi-channel array out of several single-channel ones.
// using a Stream for concurrency.
//
// For further details, please see:
// https://docs.opencv.org/4.x/de/d09/group__cudaarithm__core.html#gaac939dc3b178ee92fb6e7078f342622c
func MergeWithStream(mv []GpuMat, dst *GpuMat, stream Stream) {
	cMatArray := make([]C.GpuMat, len(mv))
	for i, r := range mv {
		cMatArray[i] = r.p
	}
	...
	C.GpuMerge(cMats, dst.p, stream.p)
}

// Split copies each plane of a multi-channel matrix into an array.
//
// For further details, please see:
// https://docs.opencv.org/4.x/de/d09/group__cudaarithm__core.html#gabe5013d55d4ff586b20393913726179e
func Split(src GpuMat) []GpuMat {
	cMats := C.struct_GpuMats{}
	defer C.GpuMats_Close(cMats)

	C.GpuSplit(src.p, &(cMats))

	mv := make([]GpuMat, cMats.length)
	for i := C.int(0); i < cMats.length; i++ {
		mv[i].p = C.GpuMats_get(cMats, i)
...
// TransposeWithStream transposes a matrix
// using a Stream for concurrency.
//
// For further details, please see:
// https://docs.opencv.org/4.x/de/d09/group__cudaarithm__core.html#ga327b71c3cb811a904ccf5fba37fc29f2
func TransposeWithStream(src GpuMat, dst *GpuMat, stream Stream) {
	C.GpuTranspose(src.p, dst.p, stream.p)
}
